...
void renderImGuiStyleEditor() {
    ...
    if (ImGui::SliderInt("Hue", &hue, 0, 255))
        setHue(static_cast<float>(hue) / 255.0);
...
void setHue(float hue) {
    ImGui::ColorConvertHSVtoRGB(hue, 85.0f / 255.0f, 84.0f / 255.0f, idle.x, idle.y, idle.z);
    ImGui::ColorConvertHSVtoRGB(hue, 221.0f / 255.0f, 191.0f / 255.0f, hover.x, hover.y, hover.z);
    ImGui::ColorConvertHSVtoRGB(hue, 220.0f / 255.0f, 153.0f / 255.0f, active.x, active.y, active.z);
    ImGui::ColorConvertHSVtoRGB(hue, 152.0f / 255.0f, 111.0f / 255.0f, dark.x, dark.y, dark.z);
    setupImGuiAccent(&ImGui::GetStyle());
}
