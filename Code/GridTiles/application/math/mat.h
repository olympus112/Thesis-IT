...
Matrix<T, Height, Width1 + Width2> joinHorizontal(const Matrix<T, Height, Width1>& mat1,
                                                  const Matrix<T, Height, Width2>& mat2) {
...
Matrix<T, Height1 + Height2, Width> joinVertical(const Matrix<T, Height1, Width>& mat1,
                                                 const Matrix<T, Height2, Width>& mat2) {
...
Matrix<T, Height1 + Height2, Width1 + Width2> join(const Matrix<T, Height1, Width1>& topLeft,
                                                   const Matrix<T, Height1, Width2>& topRight,
                                                   const Matrix<T, Height2, Width1>& botLeft,
                                                   const Matrix<T, Height2, Width2>& botRight) {
...
template <typename T, std::size_t Height, std::size_t Width>
Matrix<T, Height + 1, Width + 1> join(const Matrix<T, Height, Width>& topLeft, const Vector<T, Height>& topRight,
                                      const Vector<T, Width>& botLeft, const T& botRight) {
...
template <typename T, std::size_t Height1, std::size_t Height2, std::size_t Width1, std::size_t Width2>
Matrix<T, Height1 + Height2, Width1 + Width2> joinDiagonal(const Matrix<T, Height1, Width1>& topLeft,
                                                           const Matrix<T, Height1, Width2>& botRight) {
...
Matrix<T, ResultHeight, ResultWidth> operator*(const Matrix<T, ResultHeight, IntermediateSize>& m1,
                                               const Matrix<T, IntermediateSize, ResultWidth>& m2) {
...
*/
template <typename T, std::size_t Size>
SymmetricMatrix<T, Size> mulSymmetricLeftRightTranspose(const SymmetricMatrix<T, Size>& sym,
                                                        const SquareMatrix<T, Size>& m) {
...
*/
template <typename T, std::size_t Size>
SymmetricMatrix<T, Size> mulSymmetricLeftTransposeRight(const SymmetricMatrix<T, Size>& sym,
                                                        const SquareMatrix<T, Size>& m) {
...
auto getSubMatrix(const MatrixT& matrix, size_t rowOffset = 0,
                  size_t colOffset = 0) -> Matrix<
	typename std::remove_reference<decltype(matrix(0, 0))>::type, R, C> {
...
template <size_t R, size_t C, typename MatrixT>
void setSubMatrix(MatrixT& matrix,
                  const Matrix<typename std::remove_reference<decltype(std::declval<MatrixT>()(0, 0))>::type, R, C>&
                  value, size_t rowOffset = 0, size_t colOffset = 0) {
	assert(
		rowOffset >= 0 && rowOffset + value.height() <= matrix.height() && colOffset >= 0 && colOffset + value.width
		() <= matrix.width());
	for (size_t r = 0; r < R; r++) {
		for (size_t c = 0; c < C; c++) {
...
void setSubMatrix(MatrixT& matrix,
                  const SymmetricMatrix<typename std::remove_reference<decltype(std::declval<MatrixT>()(0, 0)
                                        )>::type, Size>& value, size_t rowOffset = 0, size_t colOffset = 0) {
	assert(
		rowOffset >= 0 && rowOffset + value.height() <= matrix.height() && colOffset >= 0 && colOffset + value.width
		() <= matrix.width());
...
template <size_t Size, typename MatrixT>
void setSubMatrix(MatrixT& matrix,
                  const DiagonalMatrix<typename std::remove_reference<decltype(std::declval<MatrixT>()(0, 0))>::type
                                       , Size>& value, size_t rowOffset = 0, size_t colOffset = 0) {
	assert(
		rowOffset >= 0 && rowOffset + value.height() <= matrix.height() && colOffset >= 0 && colOffset + value.width
		() <= matrix.width());
	for (size_t r = 0; r < Size; r++) {
